...
<html>
	...
	<body>
		<article class="page-container">
			<form action="<%=basePath%>/singerType/${requestUrl}.do" method="post" class="form form-horizontal" id="edit_form" onsubmit="return requestForm()">
				<div class="row cl">
					<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>歌手类型名：</label>
					<div class="formControls col-xs-8 col-sm-9">
						<input type="text" class="input-text"  value="${singerType.singer_type}" 
							id="singer_type" name="singer_type">
					</div>
				</div>
				<div class="row cl">
					<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
						<input type="hidden" id="singerTypeId" value="${singerType.id}">
	...
	<script type="text/javascript">
		...
		function requestForm(){
			...
			if(singerType==null||singerType==''){
				layer.msg("请输入歌手类型名称!");
				return false;
			}
			//歌曲类型编号
			var singerTypeId=$("#singerTypeId").val();
			
			//如果不存在编号则是新增,否则的话是修改
			if(singer_type_id!=''){
				requestData={"singerTypeId":singerTypeId,'singerType':singerType};
			}else{
				requestData={'singerType':singerType};
			}
			//获取请求路径
			var form=$("#edit_form");
