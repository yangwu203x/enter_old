...
<html>
	...
	<body>
		<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>热门歌曲管理<span class="c-gray en">&gt;</span>热门歌曲列表<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
		<div class="page-container">
			...
			<table class="table table-border table-bordered table-hover table-bg table-sort">
				...
				<tbody>
					<c:forEach items="${hotSongs}" var="hotSong">
						<tr class="text-c">
							<td><input type="checkbox" value="${hotSong.hotId}" name="hot_id"></td>
							<td>${hotSong.hotSongId}</td>
							<td>${hotSong.songName}</td>
							<td>${hotSong.rank}</td>
							<td class="td-manage">
								<c:if test="${sessionScope.role.update==3}">
									<a title="编辑" href="javascript:;" onclick="edit('${hotSong.hotId}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
								</c:if>
								<c:if test="${sessionScope.role.del==2}">
									<a title="删除" href="javascript:;" onclick="deleteRank(this,'${hotSong.hotId}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
								</c:if>
							</td>
						</tr>
					</c:forEach>
	...
	function edit(id){
		//请求的路径
		var url="<%=basePath%>/hotSong/hotSongEdit.do";
		var title="修改热门热门歌曲";
		var width="";
		var height="510";
		url+="?viewName=hotSong_edit&operateType=2&hotId="+id;
	...
	function add(){
		...
		var width="";
		var height="510";
		url+="?viewName=hotSong_edit&operateType=1";
		layer_show(title,url,width,height);
	}
	...
	function deleteRank(obj,id){
		...
		layer.confirm('确认要删除吗？',function(index){
			...
			var status=false;
			//访问后台的请求路径
			var requestUrl="<%=basePath%>/hotSong/delHotSongs.do";
			//请求后台的数据
