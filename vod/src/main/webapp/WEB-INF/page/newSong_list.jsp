...
<html>
	...
	<body>
		<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>新歌管理<span class="c-gray en">&gt;</span>新歌列表<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
			...
			<table class="table table-border table-bordered table-hover table-bg table-sort">
				...
				<tbody>
					<c:forEach items="${newSongs}" var="newSong">
						<tr class="text-c">
							<td><input type="checkbox" value="${newSong.newId}" name="new_id"></td>
							<td>${newSong.newSongId}</td>
							<td>${newSong.songName}</td>
							<td class="td-manage">
								<c:if test="${sessionScope.role.update==3}">
									<a title="编辑" href="javascript:;" onclick="edit('${newSong.newId}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
								</c:if>
								<c:if test="${sessionScope.role.del==2}">
									<a title="删除" href="javascript:;" onclick="deleteRank(this,'${newSong.newId}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
								</c:if>
							</td>
						</tr>
					</c:forEach>
	...
	function edit(id){
		//请求的路径
		var url="<%=basePath%>/newSong/newSongEdit.do";
		var title="修改新歌信息";
		var width="";
		var height="510";
		url+="?viewName=newSong_edit&operateType=2&newId="+id;
	...
	function add(){
		...
		var width="";
		var height="510";
		url+="?viewName=newSong_edit&operateType=1";
		layer_show(title,url,width,height);
	}
	...
	function batchDel(){
		...
		var deleteNos="";
		//获取复选框元素
		var allNode=document.getElementsByName("new_id");
		//保存待删除的一组复选框元素
	...
	function deleteRank(obj,id){
		...
		layer.confirm('确认要删除吗？',function(index){
			...
			var status=false;
			//访问后台的请求路径
			var requestUrl="<%=basePath%>/newSong/delNewSongs.do";
			//请求后台的数据
			var requestData={"newId":id};
			$.ajax({
				url:requestUrl,
				type:'POST',
				data:requestData,
