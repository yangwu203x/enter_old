...
<html>
	...
	<body>
		<article class="page-container">
			<form action="<%=basePath%>/hotSong/${requestUrl}.do" method="post" class="form form-horizontal" id="edit_form" onsubmit="return requestForm()">
				<div class="row cl">
					<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>热门歌曲名称:</label>
					<div class="formControls col-xs-8 col-sm-9">
						<input type="text" class="input-text"  value="${hotSong.songName}" 
				...
				<div class="row cl">
					...
					<div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
						...
						<c:if test="${hotSong.hotSongId!=null}">
							<option value="${hotSong.hotSongId}" selected>${hotSong.hotSongId}</option>
						</c:if>
						</select>
						</span> </div>
				</div>
				...
				<div class="row cl">
					<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>热门歌曲排序：</label>
					<div class="formControls col-xs-8 col-sm-9">
						<input type="number" class="input-text" style="width:100%;" value="${hotSong.rank}" 
							id="song_priority" name="song_priority">
					</div>
				</div>
				
				<div class="row cl">
					<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
						<input type="hidden" id="hot_id" value="${hotSong.hotId}">
	...
	<script type="text/javascript">
		...
		function requestForm(){
			...
			var songPriority=$("#song_priority").val();
			if(songPriority==null||songPriority==''){
				layer.msg("请输入歌曲排序!");
				return false;
			}
			...
			//如果不存在编号则是新增,否则的话是修改
			if(hotId!=''){
				requestData={"songId":songId,"songPriority":songPriority,"hotId":hotId};
			}else{
				requestData={"songId":songId,"songPriority":songPriority};
			}
			//获取请求路径
			var form=$("#edit_form");
